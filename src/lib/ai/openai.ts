...
export function getOpenAIBaseUrl() {
  return useAiGateway() ? 'https://ai-gateway.vercel.sh/v1' : 'https://api.openai.com/v1'
}

export function getOpenAIModel() {
  const configured = process.env.OPENAI_AGENT_MODEL || 'gpt-5.6-luna'
  return useAiGateway() ? (process.env.AI_GATEWAY_MODEL || 'openai/' + configured) : configured
}

export async function getOpenAIToken() {
  const connector = useAiGateway() ? process.env.CONNECTOR_AI_GATEWAY : getOpenAIConnector()
