...
export const ORGANIZATION_TYPES = [
  ...
  { value: 'technology_partner', label: 'Technology / integration partner' },
  { value: 'service_provider', label: 'Service provider' },
  { value: 'customer', label: 'Technology customer' },
...
export async function ensureWorkspace() {
  ...
  if (existing?.org_id) {
    return {
      supabase,
      user,
      organization: Array.isArray(existing.organizations) ? existing.organizations[0] : existing.organizations,
      orgId: existing.org_id,
    }
  }

  const metadata = user.user_metadata || {}
  const baseName = (metadata.organization_name as string | undefined)?.trim()
  ...
  const organizationType = typeof metadata.organization_type === 'string' ? metadata.organization_type : null
  const organizationRoles = Array.isArray(metadata.organization_roles) ? metadata.organization_roles.filter((v): v is string => typeof v === 'string') : []
  ...
  const { data: organization, error: orgError } = await supabase
    ...
    .insert({
      ...
      onboarding_status: organizationType ? 'completed' : 'needs_setup',
      ai_autonomy_level: 'balanced',
    })
    .select('id,name,slug,plan,brand_color,logo_url,organization_type,organization_roles,onboarding_status,ai_autonomy_level')
    .single()
