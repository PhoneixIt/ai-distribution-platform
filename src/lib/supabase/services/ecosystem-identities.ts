...
export async function listExternalIdentities(
  ...
  connectionId: string,
): Promise<EcosystemExternalIdentity[]> {
  const { data, error } = await supabase
    .from('ecosystem_external_identities')
    .select('*')
...
): Promise<EcosystemExternalIdentity | null> {
  const { data, error } = await supabase
    .from('ecosystem_external_identities')
    .select('*')
    .eq('connection_id', connectionId)
    .eq('external_record_type', externalRecordType)
...
export async function upsertExternalIdentity(
  ...
  input: {
    ...
    match_method?: string
    match_confidence?: number | null
    status?: EcosystemExternalIdentity['status']
    metadata?: Record<string, unknown>
  },
): Promise<EcosystemExternalIdentity> {
  const payload = {
    ...input,
    external_record_type: input.external_record_type || 'organization',
    match_method: input.match_method || 'candidate',
  ...
  const { data, error } = await supabase
    .from('ecosystem_external_identities')
    .upsert(payload, { onConflict: 'connection_id,external_record_type,external_record_id' })
    .select('*')
    .single()
