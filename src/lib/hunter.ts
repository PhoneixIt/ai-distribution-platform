...
type HunterResponse = {
  data?: { emails?: HunterEmail[]; organization?: Record<string, unknown>; company?: Record<string, unknown> }
...
async function request(path: string): Promise<HunterResponse> {
  const response = await fetch(
    HUNTER_BASE + path + (path.includes('?') ? '&' : '?') + 'api_key=' + encodeURIComponent(apiKey()),
  ...
  if (!response.ok) {
    const detail = payload.errors?.length ? String(payload.errors[0].details || payload.errors[0].code || '') : ''
    throw new Error(detail || `Hunter request failed (HTTP ${response.status}).`)
  }
  return payload
}
...
function scoreContact(email: HunterEmail) {
  ...
  const verification = email.verification && typeof email.verification === 'object'
    ? email.verification as Record<string, unknown>
    : {}
  ...
  if (String(verification.status || email.verification_status || '').toLowerCase() === 'valid') score += 50
  return score
}
...
export async function searchContacts(domains: string[]) {
  ...
  for (let index = 0; index < uniqueDomains.length; index += 5) {
    ...
    const batchResults = await Promise.all(batch.map(async (domain) => {
      ...
      const consumed = Number(payload.meta?.credits_used ?? payload.meta?.credits_consumed ?? (emails.length ? 1 : 0))
      return { domain, emails, credits: Number.isFinite(consumed) ? consumed : 0 }
    }))
    results.push(...batchResults)
    credits += batchResults.reduce((sum, item) => sum + item.credits, 0)
  }
