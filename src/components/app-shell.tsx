    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <header className="sticky top-0 z-40 border-b border-slate-800/90 bg-slate-950/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between px-4 py-3 lg:px-6">
          <div className="flex items-center gap-3">
            <button onClick={() => setMobileOpen((v) => !v)} className="rounded-lg border border-slate-800 p-2 text-slate-300 lg:hidden" aria-label="Open navigation">☰</button>
            <Link href="/app" className="flex items-center gap-2.5">
              <span className="grid h-9 w-9 place-items-center rounded-xl bg-blue-600 text-sm font-black">AI</span>
              <span><span className="block text-sm font-semibold">AI Distribution Platform</span><span className="hidden text-[11px] text-slate-500 sm:block">Channel intelligence workspace</span></span>
            </Link>
          </div>
          <div className="flex items-center gap-3">
      ...
      <div className="mx-auto flex max-w-[1500px]">
        <aside className={`${mobileOpen ? 'block' : 'hidden'} fixed inset-y-[61px] left-0 z-30 w-72 border-r border-slate-800 bg-slate-950 px-4 py-5 lg:sticky lg:top-[61px] lg:block lg:h-[calc(100vh-61px)] lg:w-64 lg:shrink-0`}>
          ...
          <div className="mt-8 rounded-xl border border-blue-900/50 bg-blue-950/20 p-4">
            <p className="text-xs font-semibold uppercase tracking-wider text-blue-400">Platform flow</p>
            <p className="mt-2 text-sm font-medium">Discover → Verify → Match → Engage</p>
            <p className="mt-2 text-xs leading-5 text-slate-500">Every important company claim can retain a source and confidence signal.</p>
          </div>
        ...
        <main className="min-w-0 flex-1 px-4 py-6 lg:px-8 lg:py-8">
          {(title || subtitle) && <div className="mb-7"><h1 className="text-2xl font-semibold tracking-tight lg:text-3xl">{title}</h1>{subtitle && <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">{subtitle}</p>}</div>}
          {children}
        </main>
      </div>
    </div>
