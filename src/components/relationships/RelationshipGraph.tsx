...
import {
  ...
  updateEcosystemOrganizationRoles,
...
const relationshipLabel = (fromRole: string, toRole: string) => {
  if (!fromRole || !toRole) return 'Relationship'
  if ((fromRole === 'vendor' && toRole === 'distributor') || (fromRole === 'distributor' && toRole === 'vendor')) return 'Distribution'
  if (toRole === 'customer' || fromRole === 'customer') return 'Customer'
  if (fromRole === 'technology_partner' || toRole === 'technology_partner') return 'Technology'
  if (['reseller','var','msp','mssp','system_integrator','service_provider'].includes(fromRole) ||
      ['reseller','var','msp','mssp','system_integrator','service_provider'].includes(toRole)) return 'Channel'
  return 'Partnership'
}
...
export function RelationshipGraph({ initialRelationships, error }: Props) {
  ...
  const filteredB = organizations.filter(o => o.id !== form.from && o.display_name.toLowerCase().includes(searchB.toLowerCase()))

  const selectRelated = (id: string) => {
    const org = organizations.find(item => item.id === id)
    const role = org?.organization_roles?.find(item => roleOrder.includes(item)) || ''
    const fromRole = selectFromRole(workspaceRoles, role) || workspaceRoles[0] || ''
    const type = fromRole && role && compatibleTypes[fromRole]?.[role] ? compatibleTypes[fromRole][role] : ''
    setForm(f => ({ ...f, to: id, toRole: role, fromRole, type }))
  }

  const selectRelatedRole = (role: string) => {
    const fromRole = selectFromRole(workspaceRoles, role) || workspaceRoles[0] || ''
    const type = fromRole && role && compatibleTypes[fromRole]?.[role] ? compatibleTypes[fromRole][role] : ''
    setForm(f => ({ ...f, toRole: role, fromRole, type }))
  }
  ...
  const submit = async () => {
    ...
    try {
      ...
      const related = organizations.find(o => o.id === form.to)
      if (related && !related.organization_roles.includes(form.toRole)) {
        const updatedRelated = await updateEcosystemOrganizationRoles(supabase, related.id, [...related.organization_roles, form.toRole])
        setOrganizations(prev => prev.map(o => o.id === updatedRelated.id ? updatedRelated : o))
      }
  ...
  return <main className="mx-auto w-full max-w-7xl px-6 py-8">
    <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="text-sm font-medium text-indigo-600">Ecosystem</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-slate-950">Relationships</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Manage ecosystem relationships, lifecycle, and context from one workspace.</p></div>
    ...
    {showAdd ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4"><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-center justify-between"><h2 className="text-xl font-semibold">Add relationship</h2><button onClick={()=>setShowAdd(false)} className="text-slate-500">Close</button></div>
      <div className="mt-5 grid gap-5 sm:grid-cols-2">
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-2 max-h-40 overflow-y-auto rounded-lg border border-slate-200 bg-white">
            {filteredB.filter(o=>o.id!==form.from).length === 0 ? <p className="px-3 py-3 text-sm text-slate-500">No matching organizations. Add a new organization below.</p> : filteredB.filter(o=>o.id!==form.from).map(o=><button type="button" key={o.id} onClick={()=>selectRelated(o.id)} className={form.to===o.id ? 'block w-full border-b border-slate-100 bg-indigo-50 px-3 py-2.5 text-left text-indigo-700 last:border-0' : 'block w-full border-b border-slate-100 bg-white px-3 py-2.5 text-left text-slate-800 hover:bg-slate-50 last:border-0'}><span className="block text-sm font-medium">{o.display_name}</span>{o.website||o.country ? <span className="mt-0.5 block text-xs text-slate-500">{[o.website,o.country].filter(Boolean).join(' · ')}</span> : null}</button>)}
          </div>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3"><p className="text-xs font-medium text-slate-500">Your role</p><p className="mt-1 text-sm font-semibold text-slate-900">{roleLabelList(workspaceRoles)}</p><p className="mt-1 text-[11px] text-slate-500">Inherited from account setup</p></div>
        <label className="text-xs font-medium text-slate-600">Related role
          <select value={form.toRole} onChange={e=>selectRelatedRole(e.target.value)} disabled={!form.to} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 disabled:bg-slate-50">
            <option value="">Select related role</option>
            {(organizations.find(o=>o.id===form.to)?.organization_roles?.length ? organizations.find(o=>o.id===form.to)!.organization_roles : roleOrder).map(v=><option key={v} value={v}>{roleLabels[v]}</option>)}
          </select>
        </label>
        <div className="rounded-lg border border-indigo-100 bg-indigo-50/60 p-3 sm:col-span-2"><p className="text-xs font-medium text-indigo-700">Relationship</p><p className="mt-1 text-base font-semibold text-slate-950">{storedRelationshipLabel(form.type, form.fromRole, form.toRole)}</p><p className="mt-1 text-xs text-slate-500">Automatically derived from the workspace and related role.</p></div>
        <label className="text-xs font-medium text-slate-600">Lifecycle<select value={form.lifecycle} onChange={e=>setForm(f=>({...f,lifecycle:e.target.value}))} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900">{RELATIONSHIP_LIFECYCLE_STAGES.map(v=><option key={v} value={v}>{lifecycleLabels[v]}</option>)}</select></label>
        <label className="text-xs font-medium text-slate-600">Status<select value={form.status} onChange={e=>setForm(f=>({...f,status:e.target.value}))} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900">{RELATIONSHIP_STATUSES.map(v=><option key={v} value={v}>{v}</option>)}</select></label>
        <label className="text-xs font-medium text-slate-600">Market<select value={form.market} onChange={e=>setForm(f=>({...f,market:e.target.value}))} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900"><option value="">Select market</option>{marketOptions.map(v=><option key={v} value={v}>{v}</option>)}</select></label>
        <label className="text-xs font-medium text-slate-600">Territory<select value={form.territory} onChange={e=>setForm(f=>({...f,territory:e.target.value}))} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900"><option value="">Select territory</option>{territoryOptions.map(v=><option key={v} value={v}>{v}</option>)}</select></label>
        <label className="text-xs font-medium text-slate-600">Start date<input type="date" value={form.started} onChange={e=>setForm(f=>({...f,started:e.target.value}))} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900"/></label>
        <label className="text-xs font-medium text-slate-600">Next action<input type="datetime-local" value={form.nextAction} onChange={e=>setForm(f=>({...f,nextAction:e.target.value}))} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900"/></label>
        <label className="text-xs font-medium text-slate-600 sm:col-span-2">Notes <span className="font-normal text-slate-400">optional</span><textarea placeholder="Add context only when the source does not contain it." value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} className="mt-1 min-h-24 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400"/></label>
      </div>
