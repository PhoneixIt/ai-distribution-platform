        ...
        {profile.partnerRoles.length > 0 && (
          <div className="mt-6 border-t border-slate-100 pt-5">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {profile.partnerRoles.map(role => (
                <span key={role} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs text-slate-700">
                  {getPartnerSubtypeLabel(role)}
                </span>
              ))}
            </div>
          </div>
