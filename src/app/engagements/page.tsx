...
export default function EngagementsPage() {
  ...
  return <AppShell title="Engagements" subtitle="The relationship layer after a partner is selected. Keep introductions, status and next steps connected to the opportunity.">
    {error && <div className="mb-5 rounded-xl border border-red-200/60 bg-red-50 p-4 text-sm text-red-700">{error}</div>}
    <section className="rounded-2xl border border-slate-200 bg-slate-50 p-6">
      <div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-blue-700">Partner relationships</p><h2 className="mt-2 text-xl font-semibold">Opportunity engagements</h2><p className="mt-1 text-sm text-slate-500">Select a partner from an opportunity, then move the relationship through its engagement stages here.</p></div><Link href="/opportunities" className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold hover:bg-blue-500">Open opportunities</Link></div>
      {loading ? <div className="py-12 text-center text-sm text-slate-500">Loading engagements…</div> : items.length === 0 ? <div className="mt-6 rounded-xl border border-dashed border-slate-200 p-10 text-center"><p className="text-sm text-slate-500">No partner engagements yet.</p><p className="mt-1 text-xs text-slate-500">Run a match on an opportunity and select a partner to create the first relationship record.</p></div> : <div className="mt-6 space-y-3">{items.map((item) => <article key={item.id} className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{item.partners?.name || 'Partner'}</h3><span className="rounded-full border border-slate-200 px-2 py-1 text-[11px] capitalize text-slate-500">{item.status}</span>{item.is_primary && <span className="rounded-full border border-blue-900 px-2 py-1 text-[11px] text-blue-700">Primary</span>}</div><p className="mt-2 text-sm text-slate-500">Opportunity: {item.opportunities?.title || 'Unknown'} · {item.opportunities?.stage?.replaceAll('_', ' ') || 'stage not set'}</p><p className="mt-1 text-xs text-slate-500">Role: {item.role} · Added {new Date(item.created_at).toLocaleDateString()}</p></div><div className="flex flex-wrap items-center gap-2"><select value={item.status} onChange={(event) => void updateStatus(item.id, event.target.value)} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 outline-none focus:border-blue-500">{statuses.map((status) => <option key={status} value={status}>{status}</option>)}</select><Link href={`/opportunities/${item.opportunity_id}`} className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-700 hover:border-blue-500">Open opportunity</Link></div></div></article>)}</div>}
    </section>
  </AppShell>
}
