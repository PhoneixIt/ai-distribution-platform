...
const input='mt-2 w-full rounded-xl border border-slate-800 bg-slate-950 px-3 py-2.5 text-sm text-slate-200 outline-none focus:border-blue-500'
...
export default function PricingPage(){
 ...
 useEffect(()=>{let active=true;(async()=>{try{await load()}catch{if(active)setError('Could not load pricing workspace.')}})();return()=>{active=false}},[])
 ...
 return <AppShell title="Commercial pricing" subtitle="Maintain vendor, reseller and customer pricing inside the workspace. Pricing can feed opportunity economics.">
  <form onSubmit={submit} className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
   <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
    ...
    <label className="text-sm text-slate-300 md:col-span-2">Notes<textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} rows={2} className={input}/></label>
   </div>
   {error&&<div className="mt-5 rounded-xl border border-red-900 bg-red-950/20 p-3 text-sm text-red-300">{error}</div>}
  ...
  <section className="mt-7 rounded-2xl border border-slate-800 bg-slate-900 p-6">
   <div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-blue-400">Price book</p><h2 className="mt-1 text-xl font-semibold">Workspace pricing records</h2></div><span className="text-xs text-slate-500">{records.length} records</span></div>
   {loading?<div className="py-10 text-center text-sm text-slate-500">Loading pricing…</div>:records.length===0?<div className="mt-5 rounded-xl border border-dashed border-slate-800 p-8 text-center text-sm text-slate-500">No pricing records yet.</div>:<div className="mt-5 overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="border-b border-slate-800 text-left text-xs uppercase tracking-wider text-slate-600"><th className="px-3 py-3">Vendor / Product</th><th className="px-3 py-3">Type</th><th className="px-3 py-3">Price</th><th className="px-3 py-3">Discount</th><th className="px-3 py-3">Margin</th><th className="px-3 py-3">Validity</th></tr></thead><tbody>{records.map(r=><tr key={r.id} className="border-b border-slate-900"><td className="px-3 py-4"><p className="font-medium text-slate-200">{r.products?.name||'Vendor-level price'}</p><p className="mt-1 text-xs text-slate-500">{r.vendors?.name||'Vendor'}</p>{r.partners?.name&&<p className="mt-1 text-xs text-slate-600">Partner: {r.partners.name}</p>}</td><td className="px-3 py-4 capitalize text-slate-400">{r.price_type}</td><td className="px-3 py-4 font-semibold text-slate-200">{r.currency} {Number(r.unit_price).toLocaleString()}</td><td className="px-3 py-4 text-slate-400">{r.discount_percent==null?'—':r.discount_percent+'%'}</td><td className="px-3 py-4 text-slate-400">{r.margin_percent==null?'—':r.margin_percent+'%'}</td><td className="px-3 py-4 text-xs text-slate-500">{r.valid_from+' → '+(r.valid_to||'open')}</td></tr>)}</tbody></table></div>}
  </section>
 </AppShell>
}
