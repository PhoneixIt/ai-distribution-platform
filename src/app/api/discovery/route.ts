...
function normalizeRequest(input: Partial<PartnerDiscoveryRequest>): PartnerDiscoveryRequest {
  ...
  return {
    ...
    serviceOrCapability: input.serviceOrCapability
      ? String(input.serviceOrCapability).trim()
      : undefined,
    vendorPartnership: input.vendorPartnership
      ? String(input.vendorPartnership).trim()
      : undefined,
    ...
    desiredCandidateCount: Math.min(
      MAX_CANDIDATES,
      Math.max(1, Number(input.desiredCandidateCount) || 10)
    ),
  }
}
...
export async function POST(request: Request) {
  ...
  if (!discoveryRequest.country || !discoveryRequest.technologyFocus) {
    return NextResponse.json(
      { error: 'Country and technology focus are required.' },
      { status: 400 }
    )
  }
  ...
  if (!discoveryRequest.partnerTypes.length) {
    return NextResponse.json(
      { error: 'At least one partner type is required.' },
      { status: 400 }
    )
  }
  ...
  if (authError || !user) {
    return NextResponse.json(
      { error: authError?.message || 'Authentication is unavailable.' },
      { status: 401 }
    )
  }
...
export async function GET() {
  ...
  if (error || !user) {
    return NextResponse.json({ error: error?.message || 'Authentication is unavailable.' }, { status: 401 })
  }
