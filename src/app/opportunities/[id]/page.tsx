...
export default function OpportunityDetailPage() {
  ...
  if (loading) return <main className="min-h-screen bg-white p-8 text-slate-500">Loading opportunity…</main>
  if (!opportunity) return <main className="min-h-screen bg-white p-8 text-slate-900"><p>Opportunity not found.</p><Link href="/opportunities" className="mt-4 inline-block text-blue-700">← Back to opportunities</Link></main>
  ...
  return (
    <AppShell title="Opportunity workspace" subtitle="Connect customer demand, channel matching and partner engagement.">
      <div className="mx-auto max-w-6xl px-0 py-0">
        <Link href="/opportunities" className="text-sm text-blue-700 hover:text-blue-700">← Opportunities</Link>
        <div className="mt-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-blue-700">Opportunity workspace</p>
            <h1 className="mt-2 text-3xl font-bold">{opportunity.title}</h1>
            <p className="mt-2 text-slate-500">{opportunity.customers?.company_name || 'Customer'} · {opportunity.preferred_region || 'Region not set'} · {opportunity.stage.replaceAll('_', ' ')}</p>
          </div>
        ...
        {message && <div className="mt-5 rounded-xl border border-emerald-900 bg-emerald-950/20 p-4 text-sm text-emerald-800">{message}</div>}
        ...
        <section className="mt-8 rounded-2xl border border-slate-200 bg-slate-50 p-6">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wider text-blue-700">Commercial bridge</p><h2 className="mt-2 text-xl font-semibold">Products and deal economics</h2><p className="mt-1 text-sm text-slate-500">Attach real catalog products and workspace pricing. Revenue is calculated from quantity, unit price and discount.</p></div><span className="rounded-full border border-slate-200 px-3 py-1 text-xs text-slate-500">{opportunityProducts.length} line items</span></div>
          <form onSubmit={addOpportunityProduct} className="mt-5 grid gap-3 md:grid-cols-5">
            <select value={lineForm.product_id} onChange={e=>setLineForm({...lineForm,product_id:e.target.value,pricing_record_id:''})} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800"><option value="">Select product</option>{products.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select>
            <select value={lineForm.pricing_record_id} onChange={e=>{const p=pricingOptions.find(x=>x.id===e.target.value);setLineForm({...lineForm,pricing_record_id:e.target.value,unit_price:p?String(p.unit_price):lineForm.unit_price,discount_percent:p?.discount_percent==null?lineForm.discount_percent:String(p.discount_percent)})}} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800"><option value="">No pricing record</option>{pricingOptions.filter(p=>!lineForm.product_id||p.product_id===lineForm.product_id).map(p=><option key={p.id} value={p.id}>{p.products&&!Array.isArray(p.products)?p.products.name:'Product'} · {p.price_type} · {p.currency} {Number(p.unit_price).toLocaleString()}</option>)}</select>
            <input type="number" min="0.01" step="0.01" value={lineForm.quantity} onChange={e=>setLineForm({...lineForm,quantity:e.target.value})} placeholder="Quantity" className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800"/>
            <input type="number" min="0" step="0.01" value={lineForm.unit_price} onChange={e=>setLineForm({...lineForm,unit_price:e.target.value})} placeholder="Unit price" className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800"/>
            <div className="flex gap-2"><input type="number" min="0" max="100" step="0.01" value={lineForm.discount_percent} onChange={e=>setLineForm({...lineForm,discount_percent:e.target.value})} placeholder="Discount %" className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800"/><button disabled={lineSaving||!lineForm.product_id} className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold disabled:opacity-50">{lineSaving?'Adding…':'Add'}</button></div>
          </form>
          {lineError&&<p className="mt-3 text-xs text-red-700">{lineError}</p>}
          {opportunityProducts.length?<div className="mt-5 overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="border-b border-slate-200 text-left text-xs uppercase tracking-wider text-slate-500"><th className="px-3 py-3">Product</th><th className="px-3 py-3">Qty</th><th className="px-3 py-3">Unit</th><th className="px-3 py-3">Discount</th><th className="px-3 py-3">Revenue</th></tr></thead><tbody>{opportunityProducts.map(item=><tr key={item.id} className="border-b border-slate-900"><td className="px-3 py-4 font-medium text-slate-800">{item.products&&!Array.isArray(item.products)?item.products.name:'Product'}</td><td className="px-3 py-4 text-slate-500">{Number(item.quantity).toLocaleString()}</td><td className="px-3 py-4 text-slate-500">{item.currency} {Number(item.unit_price).toLocaleString()}</td><td className="px-3 py-4 text-slate-500">{Number(item.discount_percent).toLocaleString()}%</td><td className="px-3 py-4 font-semibold text-slate-800">{item.currency} {Number(item.line_revenue).toLocaleString()}</td></tr>)}</tbody></table><div className="mt-4 flex justify-end text-sm font-semibold text-slate-800">Product revenue: USD {opportunityProducts.reduce((sum,item)=>sum+Number(item.line_revenue||0),0).toLocaleString()}</div></div>:<div className="mt-5 rounded-xl border border-dashed border-slate-200 p-8 text-center text-sm text-slate-500">No products attached to this opportunity yet.</div>}
        </section>

        <section className="mt-8 rounded-2xl border border-slate-200 bg-slate-50 p-6">
          <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-blue-700">AI matching layer</p><h2 className="mt-2 text-xl font-semibold">Best channel partners</h2><p className="mt-1 text-sm text-slate-500">Deterministic matching uses the structured partner and opportunity data first. AI explanation can be layered on top later.</p></div><span className="rounded-full border border-slate-200 px-3 py-1 text-xs text-slate-500">{matches.length} ranked</span></div>

          {matches.length === 0 ? <div className="mt-6 rounded-xl border border-dashed border-slate-200 p-10 text-center"><p className="text-sm text-slate-500">No matches yet.</p><p className="mt-1 text-xs text-slate-500">Run matching to compare your opportunity against active partner profiles.</p></div> : <div className="mt-6 space-y-4">{matches.map((match) => <article key={match.id} className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between"><div className="flex-1"><div className="flex flex-wrap items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-slate-100 text-xs font-bold">{match.rank}</span><h3 className="text-lg font-semibold">{match.partners?.name || 'Partner'}</h3><span className="rounded-full border border-slate-200 px-2 py-1 text-[11px] text-slate-500">{match.status}</span>{match.partners?.is_verified && <span className="rounded-full border border-emerald-900 px-2 py-1 text-[11px] text-emerald-700">Verified</span>}</div><p className="mt-2 text-sm text-slate-500">{match.match_reason || 'Structured data indicates potential fit.'}</p><div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-500">{match.strengths.slice(0, 4).map((item) => <span key={item} className="rounded-full border border-slate-200 px-2.5 py-1">{item}</span>)}</div></div><div className="w-full lg:w-80"><div className="flex items-end justify-between"><span className="text-xs uppercase tracking-wider text-slate-500">Match</span><span className="text-3xl font-bold text-blue-700">{Math.round(Number(match.match_score))}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-blue-500" style={{ width: `${Math.max(0, Math.min(100, Number(match.match_score)))}%` }} /></div><div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs"><Mini label="Capability" value={match.capability_fit_score} /><Mini label="Industry" value={match.industry_fit_score} /><Mini label="Geography" value={match.geography_fit_score} /><Mini label="Size" value={match.company_size_fit_score} /></div></div></div><div className="mt-5 grid gap-4 border-t border-slate-200 pt-4 md:grid-cols-2"><div><p className="text-xs uppercase tracking-wider text-slate-500">Recommended action</p><p className="mt-1 text-sm text-slate-700">{match.recommended_action || 'Review the partner profile before outreach.'}</p></div><div><p className="text-xs uppercase tracking-wider text-slate-500">Risks / missing capabilities</p><p className="mt-1 text-sm text-slate-500">{[...match.risks, ...match.missing_capabilities].slice(0, 3).join(' · ') || 'None recorded'}</p></div></div><div className="mt-5 flex flex-wrap gap-3 border-t border-slate-200 pt-4"><button onClick={() => match.partners && void selectPartner(match.partners.id)} disabled={!match.partners || selecting === match.partners.id} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold hover:bg-blue-500 disabled:opacity-50">{selecting === match.partners?.id ? 'Adding…' : 'Select partner'}</button>{match.partners?.website && <a href={match.partners.website} target="_blank" rel="noreferrer" className="rounded-lg border border-slate-200 px-4 py-2 text-sm text-slate-700 hover:border-blue-500">Visit website</a>}</div></article>)}</div>}
        </section>
      </div>
    </AppShell>
  )
...
function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-xs uppercase tracking-wider text-slate-500">{label}</p><p className="mt-2 truncate text-sm font-medium text-slate-800">{value}</p></div> }
function Mini({ label, value }: { label: string; value: number | null }) { return <div className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-2"><p className="text-[10px] uppercase tracking-wider text-slate-500">{label}</p><p className="mt-1 text-sm font-semibold text-slate-700">{value == null ? '—' : `${Math.round(Number(value))}%`}</p></div> }
