...
export default function MissionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!mission) return <AppShell title="Mission"><p className="text-sm text-slate-400">{error || 'Loading missionâ€¦'}</p></AppShell>
  ...
  return <AppShell title="Mission execution" subtitle="Evidence-backed work stays visible; external communication remains blocked until you explicitly approve a draft.">
    <Link href="/workflow" className="text-sm text-blue-400">â† Missions</Link>
    ...
    {mission.error_message ? <div className="mt-5 rounded-xl border border-red-900 bg-red-950/30 p-4 text-sm text-red-300">Mission failed: {mission.error_message}</div> : null}
      ...
      {drafts.map((draft) => {
        ...
        return <article key={draft.id} className="rounded-2xl border border-amber-900/50 bg-amber-950/10 p-6">
          ...
          <div className="mt-4 rounded-xl border border-slate-800 bg-slate-950 p-5">
            ...
            {dossier ? <>
              <p className="mt-1 text-sm text-slate-400">{String(dossier.company.country || 'Country unavailable')} Â· {String(dossier.company.website || 'Website unavailable')}</p>
              <p className="mt-3 text-xs text-slate-500">Qualification score: {String(intelligence.qualification_score ?? 'not recorded')}</p>
              {qualificationReasons.length ? <ul className="mt-2 space-y-1 text-sm text-slate-400">{qualificationReasons.map((item, index) => <li key={`${index}-${String(item)}`}>â€¢ {String(item)}</li>)}</ul> : null}
              {evidence.length ? <details className="mt-3"><summary className="cursor-pointer text-sm text-blue-400">Qualification evidence ({evidence.length})</summary><ul className="mt-2 space-y-2 text-sm text-slate-400">{evidence.map((item, index) => <li key={index}>{typeof item === 'string' ? item : JSON.stringify(item)}</li>)}</ul></details> : <p className="mt-2 text-sm text-slate-500">No qualification evidence was recorded.</p>}
              {dossier.recommended_action.rationale ? <p className="mt-3 text-sm text-slate-400">Next action: {String(dossier.recommended_action.rationale)}</p> : null}
            </> : <p className="mt-2 text-sm text-red-300">The draftâ€™s saved dossier could not be loaded, so review the source mission data before approving.</p>}
          </div>
          ...
          <div className="mt-4 rounded-xl border border-slate-800 bg-slate-950 p-5"><p className="text-xs uppercase tracking-wider text-slate-500">Why this is personalized</p><ul className="mt-2 space-y-2 text-sm text-slate-400">{draft.personalization_evidence.map((item) => <li key={item}>â€¢ {item}</li>)}</ul></div>
