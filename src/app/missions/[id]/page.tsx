...
export default function MissionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function startDiscovery() {
    ...
    try {
      ...
      const payload = await response.json()
      if (!response.ok) {
        if (payload.missionStage) {
          setMission((current) => current ? { ...current, status: 'failed', current_stage: payload.missionStage, error_message: payload.error || 'Discovery failed.' } : current)
        }
  ...
  return <AppShell title="Mission execution" subtitle="Evidence-backed work stays visible; external communication remains blocked until you explicitly approve a draft.">
    ...
    <section className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div><p className="text-xs uppercase tracking-wider text-blue-700">Mission</p><h1 className="mt-2 text-2xl font-semibold">{mission.objective}</h1><p className="mt-2 text-xs text-slate-500">{mission.id}</p></div>
        <span className="rounded-full border border-blue-900 px-3 py-1 text-xs text-blue-700">{mission.current_stage.replaceAll('_',' ')}</span>
      </div>
    ...
    <section className="mt-6 space-y-4">
      {drafts.map((draft) => {
        ...
        return <article key={draft.id} className="rounded-2xl border border-amber-200 bg-amber-50 p-6">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs uppercase tracking-wider text-amber-800">Human approval required</p><h2 className="mt-1 text-lg font-semibold">{contactName}</h2><p className="text-xs text-slate-500">{String(draft.contact.email || 'No email revealed')}</p></div><span className="rounded-full border border-amber-200 px-2.5 py-1 text-xs text-amber-800">{draft.status.replaceAll('_',' ')}</span></div>
          <div className="mt-4 rounded-xl border border-slate-200 bg-white p-5">
            ...
            {dossier ? <>
              <p className="mt-1 text-sm text-slate-500">{String(dossier.company.country || 'Country unavailable')} · {String(dossier.company.website || 'Website unavailable')}</p>
              <p className="mt-3 text-xs text-slate-500">Qualification score: {String(intelligence.qualification_score ?? 'not recorded')}</p>
              {qualificationReasons.length ? <ul className="mt-2 space-y-1 text-sm text-slate-500">{qualificationReasons.map((item, index) => <li key={`${index}-${String(item)}`}>• {String(item)}</li>)}</ul> : null}
              {evidence.length ? <details className="mt-3"><summary className="cursor-pointer text-sm text-blue-700">Qualification evidence ({evidence.length})</summary><ul className="mt-2 space-y-2 text-sm text-slate-500">{evidence.map((item, index) => <li key={index}>{typeof item === 'string' ? item : JSON.stringify(item)}</li>)}</ul></details> : <p className="mt-2 text-sm text-slate-500">No qualification evidence was recorded.</p>}
              {dossier.recommended_action.rationale ? <p className="mt-3 text-sm text-slate-500">Next action: {String(dossier.recommended_action.rationale)}</p> : null}
            </> : <p className="mt-2 text-sm text-red-700">The draft’s saved dossier could not be loaded, so review the source mission data before approving.</p>}
          </div>
          <div className="mt-5 rounded-xl border border-slate-200 bg-white p-5"><p className="text-xs uppercase tracking-wider text-slate-500">Subject</p><p className="mt-1 font-medium">{draft.subject}</p><p className="mt-5 text-xs uppercase tracking-wider text-slate-500">Message</p><p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-slate-700">{draft.body}</p></div>
          <div className="mt-4 rounded-xl border border-slate-200 bg-white p-5"><p className="text-xs uppercase tracking-wider text-slate-500">Why this is personalized</p><ul className="mt-2 space-y-2 text-sm text-slate-500">{draft.personalization_evidence.map((item) => <li key={item}>• {item}</li>)}</ul></div>
          {approval?.status === 'pending' ? <div className="mt-4 flex flex-wrap gap-2"><button disabled={!!busy} onClick={() => action(draft.id,'/api/missions/' + mission.id + '/approvals/' + approval.id,{action:'reject'})} className="rounded-lg border border-slate-200 px-4 py-2 text-sm">Reject</button><button disabled={!!busy} onClick={() => action(draft.id,'/api/missions/' + mission.id + '/approvals/' + approval.id,{action:'approve'})} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold">Approve draft</button></div> : null}
...
function Metric({ label, value }: { label: string; value: unknown }) { return <div className="rounded-xl bg-white p-4"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-2xl font-semibold">{String(value)}</p></div> }
