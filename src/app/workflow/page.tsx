...
export default function WorkflowPage() {
  ...
  return (
    <AppShell title="Missions" subtitle="Tell PortAi what you want to accomplish. It turns your objective into evidence-backed discovery, matching and approved action.">
      <section className="rounded-2xl border border-blue-200/50 bg-gradient-to-br from-blue-950/40 to-slate-900 p-6 lg:p-8">
        <div className="mb-5 rounded-2xl border border-blue-200/40 bg-white/50 p-4">
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-3">
        ...
        ].map(([label, title, copy]) => <div key={label} className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">{label}</p><h3 className="mt-2 font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{copy}</p></div>)}
      </section>
    </AppShell>
  )
}
