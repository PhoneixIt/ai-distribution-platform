    ...
    <AppShell title="Add vendor" subtitle="Contribute a vendor to the shared network and connect it to your workspace.">
      <div className="max-w-2xl rounded-2xl border border-slate-800 bg-slate-900 p-6">
        <form onSubmit={submit} className="space-y-5">
          ...
          {error && <div className="rounded-xl border border-red-900 bg-red-950/20 p-3 text-sm text-red-300">{error}</div>}
          <div className="flex gap-3 pt-2">
            ...
            <button type="button" onClick={() => router.back()} className="rounded-xl border border-slate-700 px-5 py-2.5 text-sm text-slate-300">Cancel</button>
          </div>
        </form>
      </div>
    </AppShell>
