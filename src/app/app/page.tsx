...
aexport default function AppDashboard() {
  ...
  return <AppShell title="Workspace overview" subtitle="A practical command center for discovering, managing and activating your channel ecosystem.">
    ...
    <section className="mt-8 rounded-2xl border border-slate-800 bg-slate-900 p-6"><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">Recent opportunities</h2><p className="mt-1 text-sm text-slate-500">Your latest channel pipeline activity.</p></div><Link href="/opportunities" className="text-sm text-blue-400 hover:text-blue-300">View all</Link></div>{recent.length === 0 ? <div className="mt-5 rounded-xl border border-dashed border-slate-800 p-8 text-center text-sm text-slate-500">No opportunities yet. Create one from the Opportunities workspace.</div> : <div className="mt-5 divide-y divide-slate-800">{recent.map(item => <Link key={item.id} href={`/opportunities?id=${item.id}`} className="flex flex-col gap-2 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium text-slate-200">{item.title}</p><p className="mt-1 text-xs text-slate-500">{item.stage.replaceAll('_',' ')} · {item.status}</p></div><span className="text-sm text-slate-400">{item.estimated_value ? `$${Number(item.estimated_value).toLocaleString()}` : 'Value not set'}</span></Link>)}</div>}</section>
  </AppShell>
}
